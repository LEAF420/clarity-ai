...
function App() {
  ...
  // Handler for successful model import
  const handleModelImportSuccess = () => {
    setIsModelImported(true)
    toast({
      title: "Model imported successfully",
      description: "Your Gemma 3n model is ready for offline use.",
    })
  }

  // Handler for going back from model import to onboarding
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                  {ollamaStatus.isOllamaMode && (
                    <button
                      onClick={() => {}}
                      className="flex items-center space-x-1 px-3 py-1.5 bg-blue-100 border border-blue-200 rounded-md hover:bg-blue-200 transition-colors cursor-pointer"
                      title="Ollama Local Mode"
                    >
                      <Shield className="h-3 w-3 text-blue-600" />
                      <span className="text-xs text-blue-800 font-medium">Ollama Local</span>
                    </button>
                  )}
                  {modelStatus.isImported && !ollamaStatus.isOllamaMode && (
                    <button
                      onClick={() => {}}
                    ...
                    >
                      ...
                      <span className="text-xs text-green-800 font-medium">100% Offline</span>
                    </button>
                  )}
                  {demoStatus.isDemoMode && !ollamaStatus.isOllamaMode && !modelStatus.isImported && (
                    <button
                      onClick={() => {}}
          ...
          <footer className="border-t bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 mt-auto">
            <div className="container mx-auto px-4 py-4">
              <div className="flex items-center justify-between text-sm text-muted-foreground">
                ...
                <div className="flex items-center space-x-4">
                  <a 
                    href="https://github.com/LEAF420/clarity-ai" 
                    target="_blank" 
